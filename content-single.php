...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header page-header">
		...
		<?php 
                    if ( of_get_option( 'single_post_image', 1 ) == 1 ) :
                        the_post_thumbnail( 'onepageshop-featured', array( 'class' => 'thumbnail' )); 
                    endif;
                  ?>
	...
	<footer class="entry-meta">
		<?php
			...
			$category_list = get_the_category_list( __( ', ', 'onepageshop' ) );

			/* translators: used between list items, there is a space after the comma */
			$tag_list = get_the_tag_list( '', __( ', ', 'onepageshop' ) );

			if ( ! onepageshop_categorized_blog() ) {
		...
		<?php onepageshop_setPostViews(get_the_ID()); ?>
