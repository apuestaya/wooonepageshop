 ...
 * @return array
 */
function onepageshop_page_menu_args( $args ) {
  $args['show_home'] = true;
  return $args;
}
add_filter( 'wp_page_menu_args', 'onepageshop_page_menu_args' );
 ...
 * @return array
 */
function onepageshop_body_classes( $classes ) {
...
add_filter( 'body_class', 'onepageshop_body_classes' );


// Mark Posts/Pages as Untiled when no title is used
add_filter( 'the_title', 'onepageshop_title' );

function onepageshop_title( $title ) {
  if ( $title == '' ) {
    return 'Untitled';
  } else {
    return $title;
 ...
 */
function onepageshop_remove_more_link_scroll( $link ) {
  $link = preg_replace( '|#more-[0-9]+|', '', $link );
  return $link;
}
add_filter( 'the_content_more_link', 'onepageshop_remove_more_link_scroll' );

/**
 * Change default "Read More" button when using the_excerpt
 */
function onepageshop_excerpt_more( $more ) {
  return ' <a class="more-link" href="'. get_permalink( get_the_ID() ) . '">Continue reading <i class="fa fa-chevron-right"></i></a>';
}
add_filter( 'excerpt_more', 'onepageshop_excerpt_more' );

/**
 * Add Bootstrap classes for table
 */
function onepageshop_add_custom_table_class( $content ) {
    return str_replace( '<table>', '<table class="table table-hover">', $content );
}
add_filter( 'the_content', 'onepageshop_add_custom_table_class' );
...
function custom_password_form() {
  ...
  $o = '<form class="protected-post-form" action="' . get_option('siteurl') . '/wp-login.php?action=postpass" method="post">
        <div class="row">
          <div class="col-lg-10">
              ' . __( "<p>This post is password protected. To view it please enter your password below:</p>" ,'onepageshop') . '
              <label for="' . $label . '">' . __( "Password:" ,'onepageshop') . ' </label>
            <div class="input-group">
              <input class="form-control" value="' . get_search_query() . '" name="post_password" id="' . $label . '" type="password">
              <span class="input-group-btn"><button type="submit" class="btn btn-primary" name="submit" id="searchsubmit" vvalue="' . esc_attr__( "Submit",'onepageshop' ) . '">' . __( "Submit" ,'onepageshop') . '</button>
              </span>
            </div>
          </div>
        </div>
...
if ( ! function_exists( 'onepageshop_social' ) ) :
/**
 * Process social links from Theme Options
 */
function onepageshop_social(){
  $output = '<div id="social" class="social">';
  $output .= onepageshop_social_item(of_get_option('social_facebook'), 'Facebook', 'facebook');
  $output .= onepageshop_social_item(of_get_option('social_twitter'), 'Twitter', 'twitter');
  $output .= onepageshop_social_item(of_get_option('social_google'), 'Google Plus', 'google-plus');
  $output .= onepageshop_social_item(of_get_option('social_youtube'), 'YouTube', 'youtube');
  $output .= onepageshop_social_item(of_get_option('social_linkedin'), 'LinkedIn', 'linkedin');
  $output .= onepageshop_social_item(of_get_option('social_pinterest'), 'Pinterest', 'pinterest');
  $output .= onepageshop_social_item(of_get_option('social_feed'), 'Feed', 'rss');
  $output .= onepageshop_social_item(of_get_option('social_tumblr'), 'Tumblr', 'tumblr');
  $output .= onepageshop_social_item(of_get_option('social_flickr'), 'Flickr', 'flickr');
  $output .= onepageshop_social_item(of_get_option('social_instagram'), 'Instagram', 'instagram');
  $output .= onepageshop_social_item(of_get_option('social_dribbble'), 'Dribbble', 'dribbble');
  $output .= onepageshop_social_item(of_get_option('social_skype'), 'Skype', 'skype');
  $output .= onepageshop_social_item(of_get_option('social_vimeo'), 'Vimeo', 'vimeo-square');
  $output .= '</div>';
  echo $output;
}
endif;
...
if ( ! function_exists( 'onepageshop_social_item' ) ) :
/**
 * Output social links on frontend.
 */
function onepageshop_social_item($url, $title = '', $icon = ''){
  if($url != ''):
...
if ( ! function_exists( 'onepageshop_footer_links' ) ) :
/**
 * footer menu (should you choose to use one)
 */
function onepageshop_footer_links() {
  // display the WordPress Custom Menu if available
  wp_nav_menu(array(
    'container'       => '',                              // remove nav container
    'container_class' => 'footer-links clearfix',   // class of container (should you choose to use it)
    'menu'            => __( 'Footer Links', 'onepageshop' ),   // nav name
    ...
    'fallback_cb'     => 'onepageshop_footer_links_fallback'  // fallback function
  ));
} /* end onepageshop footer link */
endif;

/**
 * Get Post Views - for Popular Posts widget
 */
function onepageshop_getPostViews($postID){
...
function onepageshop_setPostViews($postID) {
    $count_key = 'post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
 ...
 */
function onepageshop_footer_info() {
   $output = '<a href="http://rockscritps.com/wp/onepageshop" title="onepageshop Theme" target="_blank">onepageshop Theme</a> powered by <a href="http://wordpress.org" title="WordPress" target="_blank">WordPress</a>.';
   echo $output;
}
add_action( 'onepageshop_footer', 'onepageshop_footer_info', 30 );


if ( ! function_exists( 'get_onepageshop_theme_options' ) )  {
/**
 * Generate custom CSS output in website source from Theme Options.
 */
    function get_onepageshop_theme_options(){
...
add_action('wp_head','get_onepageshop_theme_options',10);

/**
 *  Theme Options sidebar
 */
add_action( 'optionsframework_after','onepageshop_options_display_sidebar' );

function onepageshop_options_display_sidebar() { ?>
  <!-- Twitter -->
  ...
  <div id="optionsframework-sidebar" class="metabox-holder">
    <div id="optionsframework" class="postbox">
        <h3><?php _e('Support and Documentation','onepageshop') ?></h3>
          <div class="inside">
              <div id="social-share">
                ...
                <ul>
                    ...
                    <li><a class="button" href="http://twitter.com/rockscritps/" title="Follow Rockscritps on Twitter" target="_blank"><?php printf(__('Follow on Twitter','onepageshop')); ?></a></li>
                </ul>
          </div>
      </div>
    </div>
...
add_action( 'tgmpa_register', 'onepageshop_register_required_plugins' );

function onepageshop_register_required_plugins() {
...
/**
     ...
     */
    $config = array(
        'id'           => 'onepageshop',                 // Unique ID for hashing notices for multiple instances of onepageshop.
        'default_path' => '',                      // Default absolute path to pre-packaged plugins.
        'menu'         => 'onepageshop-install-plugins', // Menu slug.
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'onepageshop' ),
            'menu_title'                      => __( 'Install Plugins', 'onepageshop' ),
            'installing'                      => __( 'Installing Plugin: %s', 'onepageshop' ), // %s = plugin name.
            'oops'                            => __( 'Something went wrong with the plugin API.', 'onepageshop' ),
            'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'onepageshop' ), // %1$s = plugin name(s).
            'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'onepageshop' ), // %1$s = plugin name(s).
            'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'onepageshop' ),
            'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'onepageshop' ),
            'return'                          => __( 'Return to Required Plugins Installer', 'onepageshop' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'onepageshop' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'onepageshop' ), // %s = dashboard link.
 ...
 */
add_action('woocommerce_before_main_content', 'onepageshop_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'onepageshop_wrapper_end', 10);

function onepageshop_wrapper_start() {
  echo '<div id="primary" class="col-md-8">';
}

function onepageshop_wrapper_end() {
  echo '</div>';
}


if ( ! function_exists( 'onepageshop_woocommerce_menucart' ) ) :
/**
 * Place a cart icon with number of items and total cost in the menu bar.
 *
 * https://gist.github.com/srikat/8264387#file-functions-php
 */
function onepageshop_woocommerce_menucart($menu, $args) {
  ...
  ob_start();
    global $woocommerce;
    $viewing_cart = __('View your shopping cart', 'onepageshop');
    $start_shopping = __('Start shopping', 'onepageshop');
    $cart_url = $woocommerce->cart->get_cart_url();
    $shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) );
    $cart_contents_count = $woocommerce->cart->cart_contents_count;
    $cart_contents = sprintf(_n('%d item', '%d items', $cart_contents_count, 'onepageshop'), $cart_contents_count);
...
add_filter('wp_nav_menu_items','onepageshop_woocommerce_menucart', 10, 2);


if ( ! function_exists( 'onepageshop_custom_favicon' ) ) :
/**
 * Add custom favicon displayed in WordPress dashboard and frontend
 */
function onepageshop_custom_favicon() {
  if ( of_get_option( 'custom_favicon' ) ) {
    echo '<link rel="shortcut icon" type="image/x-icon" href="' . of_get_option( 'custom_favicon' ) . '" />'. "\n";
  }
}
endif;
add_action( 'wp_head', 'onepageshop_custom_favicon', 0 );
add_action( 'admin_head', 'onepageshop_custom_favicon', 0 );
...
if (!function_exists('get_onepageshop_theme_options'))  {
  function get_onepageshop_theme_options(){
    ...
    print_r(get_option('onepageshop'));
    if ( of_get_option('social_color')) {
 ...
 */
function onepageshop_allow_skype_protocol( $protocols ){
    $protocols[] = 'skype';
    return $protocols;
}
add_filter( 'kses_allowed_protocols' , 'onepageshop_allow_skype_protocol' );
